...
let shuaCache = [];
// 分类排除规则
const cate_remove = ['分类排除', '碎片', '围观', '软鸭', '锦鲤', '番茄', '牛牛'];
...
const aggConfig = {
  ...
  platform: {
    百度: {
      ...
      detail_host: 'https://sv.baidu.com',
      url1: '/feedapi/v1/videoserver/playlets/list?service=bdbox',
      url2: '/haokan/ui-video/playlet/rec/detail?log=vhk&tn=1020970b&ctn=1008350n&blur=1',
      play_url: '/appui/api?cmd=video/relate&log=vhk&tn=1020970b&ctn=1008350n&blur=1',
...
async function init(cfg) {
    ...
    if (token) xingya_headers = {...aggConfig.headers.default, authorization: token};
    else xingya_headers = aggConfig.headers.default;
    return true;
}
...
async function home(filter) {
  // 过滤被排除的平台
  const filteredPlatforms = aggConfig.platformList.filter(item => {
    const isBad = cate_remove.some(word => 
      new RegExp(word, 'i').test(item.name) || new RegExp(word, 'i').test(item.id)
    );
    return !isBad;
  });
  
  const classes = filteredPlatforms.map(item => ({
    type_name: item.name, 
    type_id: item.id,
...
async function recommend() {
  ...
  const randomPlat = getRandomItem(filteredPlatforms);
  ...
  if (randomPlat.id === '百度') {
    platContentList = await getBaidu(defaultArea, 1, 10);
    ...
    const requestBody = JSON.stringify({page: 1, limit: 10, type_id: defaultArea, year: '', keyword: ''});
    const response = await request(`${platBaseConfig.host}${platBaseConfig.search}`,{method:'POST',body:requestBody});
    const res = JSON.parse(response);
    ...
    const res = JSON.parse(response);
    platContentList = res.data.list.map(item => ({
      vod_id: `星芽@${platBaseConfig.host}${platBaseConfig.url2}?theater_parent_id=${item.theater.id}`,
    ...
    const response = await request(`${platBaseConfig.host}${platBaseConfig.url1}`, {
      method:'POST',
    ...
    const response = await request(`${platBaseConfig.host}${platBaseConfig.search}`, {
      method:'POST',
      body: postData
    });
    const res = JSON.parse(response);
  ...
  return JSON.stringify({list: recommendList});
}

// 百度短剧核心函数
async function getBaidu(tid, pg, limit = 20) {
  ...
  const bdata = {'data': JSON.stringify({ data: innerData })};
  let html = await request(`${aggConfig.platform.百度.host}${aggConfig.platform.百度.url1}`, {
    method:'POST',
  ...
  return data.slice(0, limit).map(it => ({
    ...
    vod_pic: it.img || '',
    vod_remarks: it.updateStatus || "更新中",
    vod_content: it.description || ''
  }));
}

async function getXingyaHeaders() {
  if (xingya_headers && xingya_headers.authorization) return xingya_headers;
...
async function cfs(siteId, wd, pg) {
  ...
  // 检查平台是否被排除
  const platformItem = aggConfig.platformList.find(p => p.id === siteId);
  if (platformItem) {
    const isBad = cate_remove.some(word => 
  ...
  const plat = aggConfig.platform[siteId];
  ...
  switch (siteId) {
    case '百度': {
      ...
      const bdata = {'data': JSON.stringify({ data: innerData })};
      let html = await request(`${plat.host}${plat.search}`, {
        method:'POST',
        headers: aggConfig.headers.baidu,
        data: bdata,
...
async function category(tid, pg, filter, extend) {
  ...
  // 检查平台是否被排除
  const platformItem = aggConfig.platformList.find(p => p.id === tid);
  if (platformItem) {
    const isBad = cate_remove.some(word => 
      new RegExp(word, 'i').test(platformItem.name) || new RegExp(word, 'i').test(platformItem.id)
    );
  ...
  const searchKeyword = extend && extend.custom ? extend.custom : null;
  if (searchKeyword) {
    return await cfs(tid, searchKeyword, pg);
  }
  
  const plat = aggConfig.platform[tid];
  const area = filter && filter.area ? filter.area : (extend.area || ruleFilterDef[tid]?.area || '');
  
  switch (tid) {
    case '百度': {
      videos.push(...await getBaidu(area, page, 20));
      break;
    }
    case '甜圈': {
    ...
    case '牛牛': {
      ...
      const response = await request(plat.host + plat.url1, {
        method: 'POST',
        headers: aggConfig.headers.niuniu,
        body
      });
...
async function detail(id) {
  ...
  switch (platform) {
    case '百度': {
      const formData = `playlet_id=${did}&vid=undefined`;
      let html = await request(`${plat.detail_host}${plat.url2}`, {
        method: 'POST',
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
    ...
    case '牛牛': {
      const body = JSON.stringify({id: did, source: 0, typeId: 'S1', userId: '546932'});
...
async function play(flag, id, flags) {
  if (id == 'shua') {
    if (shuaCache.length == 0) {
      ...
      const response = await request('https://api.drama.9ddm.com/drama/home/search', { 
        method: 'POST', 
        headers: {
      ...
      res.data.forEach(it => {
        videos.push({
          ...
          url: it.oneId,
          shuaTitle: it.title,
          shuaDes: it.episodeCount + '集 | ' + it.description,
          shuaActions: {
            play: '围观@' + it.oneId,
          },
          errorPlayNext: true
    ...
    if (shuaCache.length < 1) {
      return null;
    }
    
    const cache = shuaCache.shift();
    
    const response = await request(`https://api.drama.9ddm.com/drama/home/shortVideoDetail?oneId=${cache.url}&page=1&pageSize=1`, {
      headers: {
        'User-Agent': 'okhttp/3.12.11',
        'content-type': 'application/json; charset=utf-8'
      }
    ...
    if (res.data && res.data.length > 0) {
      try {
        id = res.data[0].playSetting;
        const playSetting = typeof id === 'string' ? JSON.parse(id) : id;
        cache.url = playSetting.super || playSetting.high || playSetting.normal;
      } catch (e) {}
    }
  ...
  if (/百度/.test(flag)) {
    let url = `${aggConfig.platform.百度.detail_host}${aggConfig.platform.百度.play_url}`;
    let html = await request(url, {
      method: 'POST',
      headers: aggConfig.headers.baidu,
      body: "method=post&vid=" + id
    });
    
    let res = JSON.parse(html);
    let json = res["video/relate"]?.data?.cur_video;
    
    if (!json || !json.clarityUrl) {
    ...
    let urls = json.clarityUrl
      ...
      .map(item => ({
        title: item.title,
        url: item.url,
        order: clarityOrder[item.title] || 999
      }))
  ...
  if (/软鸭/.test(flag)) {
    const response = await request(`${aggConfig.platform.软鸭.host}/API/playlet/?video_id=${id}&quality=original`, {headers: aggConfig.headers.default});
    const res = JSON.parse(response);
  ...
  if (/牛牛/.test(flag)) {
    ...
    const body = JSON.stringify({episodeId, id: videoId, source: 0, typeId: 'S1', userId: '546932'});
...
async function search(wd, quick, pg) {
  ...
  const searchPromises = filteredPlatforms.map(async (platform) => {
...
async function request(url, obj) {
  if (!obj || !obj.headers) obj = {headers: {...aggConfig.headers.default}, timeout: 5000, ...obj};
...
function getRandomItem(items) {return items[Math.floor(Math.random() * items.length)];}
