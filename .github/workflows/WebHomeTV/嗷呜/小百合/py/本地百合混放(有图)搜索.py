...
class Spider(Spider):
    ...
    CHANNEL_PAGE_SIZE = 2000  # 👈 修改这个数值，即可控制频道分页的大小 (默认1000条一页)
    ...
    def init(self, extend):
        ...
        for r in raw_roots:
            ...
            for folder_key, display_label in self.scan_targets:
                ...
                if os.path.isdir(target_root_path):
                    ...
                    # 🎯 修改1：将频道结果反转实现由小到大显示
                    if r_items: 
                        r_items.reverse() # 执行倒序
                        all_results_list.append({"name": display_label, "items": r_items, "weight": 100, "path": target_root_path, "is_int": is_int})
                    try:
                        with os.scandir(target_root_path) as it:
                            for entry in it:
                                if entry.is_dir():
                                    ...
                                    if s_items: 
                                        s_items.reverse() # 执行倒序
                                        all_results_list.append({"name": entry.name, "items": s_items, "weight": 2, "path": entry.path, "is_int": is_int})
                                    try:
                                        with os.scandir(entry.path) as sub_it:
                                            for se in sub_it:
                                                if se.is_dir():
                                                    ...
                                                    if n_items: 
                                                        n_items.reverse() # 执行倒序
                                                        all_results_list.append({"name": f"{entry.name}/{se.name}*", "items": n_items, "weight": 5, "path": se.path, "is_int": is_int})
                                    except: pass
                    except: pass
        ...
        for res_obj in all_results_list:
            items = res_obj["items"]
            # 🎯 修改2：补上外存储显示☆
            ext_flag = "" if res_obj.get("is_int", True) else " ☆"
            ...
            for i in range(total_pages):
                ...
                # 构建分类名，包含外存星号标识
                cat_name = f"{res_obj['name']}{ext_flag}({total})[{i+1}/{total_pages}]"
